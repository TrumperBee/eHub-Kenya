    ...
    <div className="pt-16 min-h-screen bg-[#0D0D0D]">
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="section-heading">Seller Profile</h1>
        </div>

        <div className="card p-6 mb-6">
          <h2 className="font-heading text-lg font-bold text-white mb-4">Public Profile</h2>
          <div className="space-y-5">
            <div>
              <label className="block text-sm text-[#9E9E9E] mb-1.5">Seller Display Name</label>
            ...
            <div>
              <label className="block text-sm text-[#9E9E9E] mb-1.5">Bio</label>
        ...
        <div className="card p-6">
          <h2 className="font-heading text-lg font-bold text-white mb-4">Performance</h2>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-sm text-[#9E9E9E]">Rating</span>
              <div className="flex items-center gap-2">
                <div className="flex items-center">
                  {[1, 2, 3, 4, 5].map((star) => {
                    ...
                    return (
                      <Star key={star} size={16} fill={fill} color={fill !== 'none' ? '#D4AF37' : '#2A2A2A'} />
                    );
                  })}
                </div>
                <span className="text-sm text-white">{userProfile?.sellerRating?.toFixed(1) || '0.0'}</span>
                <span className="text-xs text-[#5C5C5C]">({userProfile?.sellerTotalRatings || 0} reviews)</span>
              </div>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-[#9E9E9E]">Total Sales</span>
              <span className="text-sm font-semibold text-white">{userProfile?.totalSales || 0}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-[#9E9E9E]">Total Purchases</span>
              <span className="text-sm font-semibold text-white">{userProfile?.totalPurchases || 0}</span>
            </div>
          </div>
        </div>
      </div>
