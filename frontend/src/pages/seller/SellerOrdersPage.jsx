...
import { ShoppingBag, AlertTriangle, ChevronUp, ChevronDown } from 'lucide-react';
...
function EmptyState({ filter, onSwitchToListings }) {
  if (filter === 'all') {
    return (
      <div className="text-center py-16">
        ...
        <button onClick={onSwitchToListings} className="btn-primary text-sm" style={{ background: '#FFF100', color: '#111' }}>
          VIEW MY LISTINGS
        </button>
      </div>
    );
  ...
  return (
    <div className="pt-[68px] min-h-screen" style={{ background: '#F5F5F5' }}>
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="rounded-xl p-4 mb-6" style={{ background: '#EFF6FF', borderLeft: '4px solid #003BFF' }}>
          ...
          {guideOpen && (
            <div className="text-xs leading-relaxed space-y-1.5" style={{ color: '#374151' }}>
              ...
              <p className="mt-1 font-semibold" style={{ color: '#C8102E' }}>⚠️ Important: Never ask buyers to pay outside this platform. Escrow protects both of you.</p>
            </div>
          )}
        </div>
        ...
        {filtered.length === 0 ? (
          <EmptyState filter={activeFilter} onSwitchToListings={() => window.location.href = '/transfer-room?tab=listings'} />
        ) : (
